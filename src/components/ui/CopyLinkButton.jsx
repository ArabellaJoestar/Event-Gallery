...
// Componente: CopyLinkButton
// Dependências: @radix-ui/react-tooltip, lucide-react, tailwindcss
// Uso:
// <CopyLinkButton url={someUrl} label="Copiar link" />
// Se url não for passado, usa window.location.href (quando disponível).
      ...
      <Tooltip.Root>
        <Tooltip.Trigger asChild>
          <button
            onClick={handleCopy}
            className={
              `inline-flex items-center gap-2 px-3 py-1.5 rounded-md shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary ${className}`
            }
            aria-label={label}
            title={label}
          >

...
const EditEventForm = ({ onSubmit, onCancel }) => {
  ...
  const removeVideo = (index) => {
    if (index < existingVideos.length) {
      ...
      setVideoFiles(prev => prev.filter((_, i) => i !== newIndex));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      removedVideos.forEach(v => data.append("removedVideos", v));
    ...
    >
      {loading ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {imagePreviews.map((preview, index) => (
                  ...
                  >
                    <img
                      src={preview}
                      alt={`Preview ${index + 1}`}
                      className={`w-full h-32 object-cover rounded-lg border-2 transition-all ${formData.principal_photo === index ? 'border-primary shadow-lg' : 'border-border'
                        }`}
                    />
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-sm font-semibold text-foreground">
              <ImageIcon className="w-4 h-4" /> Vídeos do Evento
            </label>
            ...
            <div className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-primary transition-colors">
              <input
                ...
                className="hidden"
              />
              <label htmlFor="videos" className="cursor-pointer flex flex-col items-center gap-3">
                <div className="p-3 bg-primary/10 rounded-full">
                  <Upload className="w-6 h-6 text-primary" />
                </div>
                <p className="text-foreground font-medium">Clique para enviar vídeos</p>
                <p className="text-sm text-muted-foreground">MP4, MOV, WEBM até 250MB</p>
              </label>
            </div>

            {/* Lista de vídeos existentes + novos */}
            {(existingVideos.length > 0 || videoFiles.length > 0) && (
              <ul className="mt-4 space-y-2">
                {[...existingVideos, ...videoFiles.map(v => v.name)].map((vid, index) => (
                  <li key={index} className="flex items-center justify-between bg-muted px-3 py-2 rounded-lg text-sm">
                    <span className="truncate">{typeof vid === "string" ? vid : vid.name}</span>
                    <button
                      type="button"

...
export default function GroupCard({ group, onCardClick, onGroupDeleted, isAuth }) {
    ...
    return (
        <motion.div variants={itemVariants} initial="hidden" animate="visible" className='h-full'>
            <div className="p-4 rounded-lg bg-[#38423B] text-white w-full h-[65vh] flex flex-col">
                <div className={`flex justify-between flex-col mb-5 ${group.events.length > 1 ? 'cursor-pointer' : ''}`}
                    onClick={() => group.events.length > 1 ? setExpanded(!expanded) : console.log('none')}>
                    <div className='flex justify-between w-full'>
                        <h2 className="text-2xl font-bold">{group.name}</h2>
                        {group.events.length > 1 && <div className="flex items-center gap-1 text-gray-500">
                            <span>{expanded && group.events.length > 1  ? 'Ocultar' : 'Mostrar mais'}</span>
                            {expanded ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                        </div>}
                    </div>
                    {expanded && <h3 className="text-xs">{group.description}</h3>}        
                ...
                {expanded && group.events.length > 0 && (
                    <motion.div
                        ...
                        initial="hidden"
                        animate="visible"
                        className={`grid grid-cols-${group.events.length >= 3 ? '3' : '2'} gap-8 mt-2 mb-5 overflow-y-auto flex-1 pr-2"`}
                    >
                        {group.events.map(event => (
                            <EventCard key={event.id} event={event} onClick={onCardClick} compact={true}/>
                        ))}
                    </motion.div>
                )}
                ...
                {!expanded && group.events.length >= 1 && (
                    <EventCard key={group.events[0].id} event={group.events[0]} onClick={onCardClick} compact={true}/>
                )
